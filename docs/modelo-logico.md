...
- **Atributos lógicos**: `nome` (persistido, obrigatório).
...
- **Observações**: campo `tipo` removido do modelo (D1, `decisions.md`, decisão #23) — auditoria documental não encontrou uso funcional em nenhuma regra, decisão ou entidade do projeto. Nenhum campo de saldo ou dado financeiro — Empresa é puro cadastro.
...
- **Atributos lógicos**: `nome` (persistido, obrigatório); `cliente` (FK, obrigatório); `valor_contratado` (persistido, obrigatório); `data_início` (persistido, obrigatório); `data_prevista_término` (persistido, obrigatório); `data_real_término` (persistido, opcional — só quando concluída); `status` (persistido, obrigatório — A executar/Em andamento/Pausada/Concluída, D9, `decisions.md` decisão #30).
...
- **Observações**: "Lucro por Obra", "custo direto", "custo rateado" **não são atributos** — resultado de consulta agregada sobre Lançamento Financeiro e Rateio Despesa, nunca persistidos (princípio 6/8). Valores de `status` resolvidos (D9, `decisions.md` decisão #30).
...
### 3.07 — VEÍCULO
- **Finalidade**: bem da frota — dimensão de custo independente e simultânea à de Obra; também representa alocação operacional corrente a uma Obra.
- **Atributos lógicos**: `nome/identificação` (persistido, obrigatório); `tipo` (persistido, obrigatório — Caminhão/Escavadeira/Pá carregadeira/Trator/Rolo compactador/Veículo leve/Terceiro/Outro, D10, `decisions.md` decisão #31); `empresa` (FK, obrigatório); `obra_atual` (FK, opcional — D4, `decisions.md` decisão #26).
- **Identificador lógico**: identificador único de Veículo.
- **Relacionamentos e cardinalidades**: Empresa 1:N Veículo; Veículo 1:N Lançamento Financeiro (opcional, pode coexistir com Obra); Veículo 1:N Compra Cartão (opcional); Veículo 1:N Contrato Financeiro (opcional — só Consórcio contemplado); Obra 1:N Veículo (opcional, alocação atual, via `obra_atual`).
- **Dependências**: Empresa (obrigatória); Obra (opcional, alocação atual).
- **Observações**: valores de `tipo` resolvidos (D10, `decisions.md` decisão #31). "Custo do Veículo" não é atributo — consulta agregada. `obra_atual` é fato operacional, independente da dimensão financeira; histórico de mudanças via `LOG_AUDITORIA`, sem entidade dedicada.
...
- **Atributos lógicos**: `tipo` (persistido, obrigatório, imutável — Despesa/Receita); `categoria` (FK, obrigatório); `empresa` (FK, obrigatório desde a criação — D7); `fornecedor` (FK, condicional a `tipo`=Despesa) / `cliente` (FK, condicional a `tipo`=Receita) — **mutuamente exclusivos**; `obra` (FK, opcional — mutuamente exclusivo com a existência de Rateio Despesa para este Lançamento); `veículo` (FK, opcional, pode coexistir com `obra`); `valor` (persistido, obrigatório, imutável a partir da 1ª Aplicação de Liquidação); `data_competência` (persistido, obrigatório); `vencimento` (persistido, obrigatório); `situação_administrativa` (persistido, obrigatório — Ativo/Cancelado); `origem` (persistido, obrigatório, imutável); **`status_financeiro` — atributo CALCULADO, não é coluna persistida** (derivado da soma de `APLICAÇÃO_DE_LIQUIDAÇÃO.valor_aplicado`, sempre, sem exceção — Decisão 1).
- **Identificador lógico**: identificador único de Lançamento Financeiro.
- **Relacionamentos e cardinalidades**: Categoria N:1; Empresa N:1; Fornecedor N:1 (condicional); Cliente N:1 (condicional); Obra N:1 (opcional); Veículo N:1 (opcional); Lançamento 1:N Rateio Despesa (opcional); Lançamento N:N Liquidação Financeira, via Aplicação de Liquidação; Lançamento (original) 1:N Ajuste Financeiro; Ajuste Financeiro 1:1 Lançamento (de ajuste); Parcela 1:1 Lançamento (opcional, quando a Parcela vence).
- **Dependências**: Categoria (obrigatória); Empresa (obrigatória, D7); Fornecedor ou Cliente (condicional, exatamente um).
- **Observações**: `situação_administrativa` só transiciona para Cancelado com soma de Aplicações = 0 (Decisão 2); correções após qualquer Aplicação passam exclusivamente por `AJUSTE_FINANCEIRO` (nunca reescrevendo `valor`). Vínculo direto e obrigatório com Empresa, antes lacuna catalogada como D7, foi resolvido: `empresa` deve corresponder a `veículo.empresa` quando `veículo` estiver preenchido — ver `decisions.md`, decisão #21.

### 3.10 — LIQUIDAÇÃO_FINANCEIRA
- **Finalidade**: evento de decisão/execução do pagamento ou recebimento de um ou mais Lançamentos.
- **Atributos lógicos**: `tipo` (persistido, obrigatório — Pagamento/Recebimento, imutável); `data_efetiva` (persistido, obrigatório, imutável); `valor` (persistido, obrigatório, imutável); `conta_bancária` (FK, obrigatório, imutável) — todos imutáveis desde a criação (D12, `decisions.md` decisão #33).
...
- **Observações**: soma das Aplicações vinculadas pode ser **menor** que `valor` (caso de Fatura mista — diferença explicável, não divergência), mas não deveria ser maior (inferência). Mutabilidade após criação resolvida (D12, `decisions.md` decisão #33) — imutável, mesma família de `APLICAÇÃO_DE_LIQUIDAÇÃO`.
...
- **Atributos lógicos**: `lançamento_original` (FK, obrigatório, imutável); `lançamento_ajuste` (FK, obrigatório, imutável); `tipo_ajuste` (persistido, obrigatório, imutável — Estorno/Reembolso/Crédito/Ajuste); `valor` (persistido, obrigatório, imutável); `data` (persistido, obrigatório, imutável); `usuário` (FK, obrigatório); `observação` (persistido, opcional, imutável) — os quatro últimos imutáveis por D13, `decisions.md` decisão #34.
...
- **Observações**: criação **exclusivamente humana** — `AÇÃO_PROPOSTA_IA` nunca gera Ajuste, em nenhum nível de sensibilidade (Decisão 8). Regras de alteração resolvidas (D13, `decisions.md` decisão #34) — imutável desde a criação, mesma família de `LIQUIDAÇÃO_FINANCEIRA` (D12). Exclusão física coberta pela regra geral de entidades de fato financeiro (`arquitetura-fisica-banco.md` §7).
...
- **Observações**: soma de `valor_rateado` deve corresponder exatamente a `valor` do Lançamento **quando o rateio estiver completo**, tolerância restrita a arredondamento de menor unidade monetária — nunca tolerância de negócio (Decisão 10). D3 resolvida (`decisions.md`, decisão #25): rateio pode ficar incompleto por tempo indeterminado — modelo lógico não impõe fechamento obrigatório no ato do registro, consistente com a decisão.
...
- **Atributos lógicos**: `origem` (persistido, obrigatório, imutável — Compra Cartão/Contrato Financeiro, discriminador); `número` (persistido, obrigatório, imutável); `total` (persistido, obrigatório, imutável); `valor` (persistido, obrigatório); `vencimento` (persistido, obrigatório); `fatura` (FK, opcional — só aplicável quando `origem`=Compra Cartão; permanente uma vez atribuído). Sem campo `status` — removido (D11, `decisions.md` decisão #32).
...
- **Observações**: "estado" da Parcela (vencida ou não, gerou Lançamento ou não) é sempre calculado em consulta a partir de `vencimento` e do vínculo com Lançamento Financeiro — nunca persistido (D11, `decisions.md` decisão #32). Ausência de `fatura` já representa "aguardando" (Decisão 7) — dimensão independente, sempre foi.
...
- **Observações**: regra 29 — `categoria` e `classificação` nunca fundidas na mesma sugestão. Mecanismo físico de `entidade_alvo` foi definido separadamente como referência polimórfica (B4, `decisions.md` decisão #20).
...
- **Atributos lógicos**: `tipo_ação` (persistido, obrigatório, imutável — catálogo ainda não fechado, ver I1); `dados_propostos` (persistido, obrigatório, imutável — **estrutura variável conforme `tipo_ação`**, forma física ainda em aberto); `nível_sensibilidade` (persistido, obrigatório, imutável — Baixo/Médio/Alto); `status` (persistido, obrigatório, mutável — uma única transição, mais a transição condicional "Aguardando Empresa" → Pendente — D7); `empresa` (FK, condicional — nulo enquanto `status` = "Aguardando Empresa", obrigatório para confirmação — D7).
- **Identificador lógico**: identificador único de Ação Proposta IA.
- **Relacionamentos e cardinalidades**: Usuário 1:N Ação Proposta IA; Empresa 1:N Ação Proposta IA (condicional, D7); Ação Proposta IA 0:1 registro real gerado (**referência genérica de escopo fechado**, restrita às entidades oficialmente permitidas para IA — `AJUSTE_FINANCEIRO` explicitamente excluído).
- **Dependências**: Usuário (para confirmação); Empresa (condicional, D7).
- **Observações**: `AJUSTE_FINANCEIRO` está **definitivamente fora** do catálogo de ações possíveis, em qualquer nível de sensibilidade (Decisão 8). Nível Alto exige barreira reforçada cujo mecanismo é reautenticação (senha) — A8, `decisions.md`, decisão #22; nenhuma alteração de schema decorre desta decisão. A dependência circular entre esta entidade e a permissão por escopo de Empresa (Achado 5, auditoria sistêmica) foi resolvida — ver `decisions.md`, decisão #21.
...
- **Observações**: **entidade 100% imutável após criação** — a única do modelo inteiro nessa condição. Mecanismo físico da referência genérica foi definido separadamente como referência polimórfica (B4, `decisions.md` decisão #20); a lista fechada de entidades auditáveis ainda não foi compilada como artefato único (tarefa de organização documental, não pendência de modelagem).
...
- **B4 — Técnica de implementação do vínculo genérico** (`LOG_AUDITORIA.entidade`/`id`; `SUGESTÃO_IA.entidade_alvo`; "registro real" de `AÇÃO_PROPOSTA_IA`). **Resolvida** separadamente como referência polimórfica, sem FK nativa (`decisions.md`, decisão #20) — este documento representava essas referências como "FK condicional a um conjunto fechado de tabelas" no momento da redação, agora substituído pela decisão oficial.
- **B3 — Estratégia de índice**. **Resolvida** em nível arquitetural (`decisions.md` decisão #37) — critério objetivo (consulta/regra já documentada), não lista de colunas; `obra_id`, `veiculo_id`, `data_competência` em `LANÇAMENTO_FINANCEIRO` permanecem como exemplos do critério, aplicados na implementação de cada módulo.
- **D9 — Enumeração de `OBRA.status`**. **Resolvida** separadamente (A executar/Em andamento/Pausada/Concluída, `decisions.md`, decisão #30) — este documento representava o campo como "valores ainda não enumerados" no momento da redação, agora substituído pela decisão oficial; implementado como `CHECK` (mesmo padrão dos demais campos enumerados do projeto).
- **D10 — Enumeração de `VEÍCULO.tipo`**. **Resolvida** separadamente (Caminhão/Escavadeira/Pá carregadeira/Trator/Rolo compactador/Veículo leve/Terceiro/Outro, `decisions.md`, decisão #31) — este documento representava o campo como "valores ainda não enumerados" no momento da redação, agora substituído pela decisão oficial; implementado como `CHECK` (mesmo padrão dos demais campos enumerados do projeto).
- **D11 — Existência de `PARCELA.status`**. **Resolvida** separadamente como remoção do campo (`decisions.md`, decisão #32) — este documento representava o campo como "valores ainda não enumerados" no momento da redação; a decisão final não enumerou valores, removeu o atributo. Estado da Parcela agora sempre calculado, nunca persistido.
- **D12 — Mutabilidade de `LIQUIDAÇÃO_FINANCEIRA`**. **Resolvida** (imutável desde a criação, `decisions.md` decisão #33) — este documento representava a mutabilidade como pendência aberta no momento da redação; a tabela física é append-only para esta entidade (sem `UPDATE` previsto), mecanismo exato de bloqueio ainda não escolhido.
- **D13 — Regras de alteração/exclusão de `AJUSTE_FINANCEIRO`**. **Resolvida** (imutável desde a criação, exclusão coberta por regra geral já existente — `decisions.md` decisão #34) — este documento representava ambas como pendência aberta no momento da redação; a tabela física é append-only para esta entidade (sem `UPDATE` previsto), mecanismo exato de bloqueio ainda não escolhido.
...
*Próximo passo da Fase 3, quando autorizado: seguir para a etapa seguinte de modelagem física (escolha de SGBD e tradução deste modelo lógico em schema físico), condicionada à pendência B1 listada acima — B3 e B4 já resolvidas (`decisions.md`, decisões #37 e #20).*
