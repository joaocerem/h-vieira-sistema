...
**Quem altera**: não definido explicitamente — ver Seção 7.
...
| `tipo_ajuste` | Natureza do ajuste | Lista de valores (Estorno / Reembolso / Crédito / Ajuste) | Sim | Nenhum | Não definido | Usuário | Um dos quatro valores | Os quatro convivem como valores do mesmo campo, sem diferença estrutural entre eles (Seção 9 do conceitual) |
| `valor` | Valor do ajuste | Valor monetário | Sim | Nenhum | Não definido | Usuário | Deve ser um valor monetário válido | — |
| `data` | Data do ajuste | Data | Sim | Nenhum | Não definido | Usuário | Deve ser uma data válida | — |
| `usuário` | Usuário responsável pelo ajuste | Referência para outra entidade (Usuário) | Sim | Nenhum | Não | — | Deve referenciar um Usuário existente | É a única entidade do catálogo oficial do conceitual que já previa explicitamente um campo de referência a usuário responsável, antes mesmo da pendência 5 ser resolvida |
| `observação` | Nota livre sobre o motivo/contexto do ajuste | Texto | Não definido explicitamente — inferência: opcional, por ser um campo de anotação livre | Nenhum | Sim, por correção de anotação — inferência, não confirmado | Usuário | Nenhuma | — |
...
- **Regras de alteração**: não definidas explicitamente no conceitual.
- **Regras de exclusão**: não definidas — dado que representa formalmente uma correção financeira já ocorrida, exclusão física seria incompatível com o princípio "nada desaparece" — observação, não regra confirmada.
...
| Imutáveis | `lançamento_original`, `lançamento_ajuste` (por inferência estrutural forte — o vínculo formal não faz sentido de negócio se puder ser reatribuído) |
...
**Esta entidade depende de alguma decisão ainda pendente? Não diretamente entre as 14 pendências numeradas**, mas três observações:
...
- Regras de alteração e exclusão desta entidade não estão definidas em lugar nenhum do conceitual — não é uma das 14 pendências numeradas, mas é uma lacuna real identificada nesta modelagem, sem urgência aparente dado que o padrão observado em toda a cadeia financeira é de imutabilidade de eventos já ocorridos.
