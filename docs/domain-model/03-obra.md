...
**Quem consulta**: `LANÇAMENTO_FINANCEIRO` (para exibir Obra associada), `RATEIO_DESPESA`, o módulo de Balanço/relatórios (para calcular custo e lucro por Obra), `CLIENTE` (para listar Obras de um Cliente).
...
Depende de `CLIENTE` (obrigatório). É referenciada por `LANÇAMENTO_FINANCEIRO` (opcionalmente, via `obra_id`) e por `RATEIO_DESPESA` (obrigatoriamente, quando existe rateio para essa Obra).

---

## 7. Checklist de decisões pendentes

**Esta entidade depende de alguma decisão ainda pendente? Sim, uma — não catalogada nas 14 pendências numeradas do conceitual, identificada agora durante esta modelagem.**

- **Qual decisão**: os valores válidos do campo `status` de Obra não estão enumerados em nenhum lugar do conceitual (diferente de `LANÇAMENTO_FINANCEIRO.status`, cujos valores são explicitados na Seção 2 do conceitual — Aberto/Parcial/Pago/Cancelado).
- **Por que**: a existência de `data_prevista_término` e `data_real_término` sugere, no mínimo, uma distinção entre "em andamento" e "concluída" — mas não há confirmação textual de quantos estados existem, nem se há estados adicionais (ex. "paralisada", "cancelada"). Sem essa definição, o campo não pode ser modelado com precisão além de "lista de valores a definir".
- **O que muda na entidade**: o conjunto de valores válidos de `status`, e possivelmente uma regra de transição entre eles (ex. se "concluída" exige `data_real_término` preenchida) — hoje não modelada por falta de base textual.
